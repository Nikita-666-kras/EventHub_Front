...
onMounted(async () => {
  ...
  try {
    ...
    // console.log('[qeqe]'+eventsRes.data)// заменяй на твой endpoint
    const allEvents = eventsRes.data || []
    const now = new Date()

    const past = allEvents.filter(event => new Date(event.endDateAndTime) < now)
    const upcoming = allEvents.filter(event => new Date(event.startDateAndTime) >= now)

    soloEvents.value = past.filter(event => event.grouping === 'solo' || event.grouping === 'Только соло')
    groupEvents.value = past.filter(event => event.grouping === 'group' || event.grouping === 'Только группы')
